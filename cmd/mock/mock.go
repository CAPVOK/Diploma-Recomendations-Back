...
func seed(db *gorm.DB) error {
	...
	courses := []domain.Course{
		{Name: "Frontend-разработка", Description: "HTML, CSS, JavaScript и React"},
		{Name: "Backend-разработка", Description: "HTTP, Go, базы данных"},
	}
	if err := db.WithContext(ctx).Create(&courses).Error; err != nil {
		return err
	}
	...
	if err := db.WithContext(ctx).Create(&beTests).Error; err != nil {
		return err
	}

	/* 4) Привязка вопросов к тестам (по 10 подряд) */
	linkQ := func(test *domain.Test, start int) error {
		return db.Model(test).
			Association("Questions").
			Append(qs[start : start+10])
	}
	for i := 0; i < 3; i++ {
		if err := linkQ(&feTests[i], i*10); err != nil {
			return err
		}
		if err := linkQ(&beTests[i], 30+i*10); err != nil {
			return err
		}
	}

	/* 5) Привязка тестов к курсам */
	if err := db.Model(&courses[0]).Association("Tests").Append(&feTests); err != nil {
		return err
	}
	if err := db.Model(&courses[1]).Association("Tests").Append(&beTests); err != nil {
		return err
	}
...
func questions() []domain.Question {
	return []domain.Question{
		...
		{Title: "Какой HTML-тег создаёт абзац?", Type: domain.Single,
			Variants: j(map[string]string{"a": "<p>", "b": "<div>", "c": "<br>", "d": "<span>"}), Answer: j("a")},
		{Title: "Сколько уровней заголовков (h1–h?) в HTML?", Type: domain.Number,
			Variants: j(nil), Answer: j(6)},
		{Title: "CSS-свойство для цвета текста", Type: domain.Text,
			Variants: j(nil), Answer: j("color")},
		{Title: "Что из списка — flex-контейнер?", Type: domain.Multiple,
			Variants: j(map[string]string{"a": "display:flex", "b": "display:grid", "c": "float:left", "d": "position:absolute"}),
			Answer:   j([]string{"a"})},
		{Title: "Какой атрибут задаёт альтернативный текст картинки?", Type: domain.Single,
			Variants: j(map[string]string{"a": "src", "b": "alt", "c": "title", "d": "href"}), Answer: j("b")},
		{Title: "Расширение файла таблиц стилей", Type: domain.Single,
			Variants: j(map[string]string{"a": ".html", "b": ".css", "c": ".js", "d": ".png"}), Answer: j("b")},
		{Title: "Аббревиатура HTML расшифровывается как…", Type: domain.Text,
			Variants: j(nil), Answer: j("hypertext markup language")},
		{Title: "Единица rem основывается на…", Type: domain.Single,
			Variants: j(map[string]string{"a": "текущий font-size", "b": "корневой font-size", "c": "ширину вьюпорта", "d": "px"}),
			Answer:   j("b")},
		{Title: "Метатег для адаптивной вёрстки", Type: domain.Text,
			Variants: j(nil), Answer: j("<meta name=\"viewport\" …>")},
		{Title: "CSS-свойство, создающее тень блока", Type: domain.Text,
			Variants: j(nil), Answer: j("box-shadow")},
		...
		{Title: "Какая функция ставит колбек в микротаски?", Type: domain.Text,
		...
		{Title: "Ключевой проп для списков в React", Type: domain.Text,
